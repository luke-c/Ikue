...
public class FavouritesFragment extends Fragment implements GetFavouritesAsyncCallbacks {
    ...
    public void onResume() {
        ...
        // User may have added a favourite when they come back to the favourites list, so
        // re-query and get all favourites again

        // As onResume is always called we only need to do the query here. This will also be called
        // Whenever the user navigates back so we can ensure we always have the correct favourites
        task = new GetFavouritesTask(listener, helper).execute();
    }
